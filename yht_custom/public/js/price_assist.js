...
//
// Price Assist — one dialog answering "what should I charge for this line?"
//
// Deliberately NOT built like sf_trading.last_selling_rate or the
// last_purchase_rate app: those inject a button by searching the grid toolbar
// through four fallback selectors and tracking clicks with capture-phase
// listeners, which breaks whenever frappe touches its grid markup. This hangs off
// the documented per-row API instead:
//   * a row-level button via `frm.fields_dict.items.grid.add_custom_button` is
//     still DOM-dependent, so we use the supported `Menu > ` primary action plus
//     an indicator on the rate field, and read the CURRENT row from
//     frappe.ui.form.get_open_grid_form(), which is a public helper.
...
yht_custom.price.show = function (frm, row, d) {
	...
	dialog.$wrapper.find(".yht-history").on("click", function () {
		...
		frappe.call({
			...
			args: { item_code: d.item_code, company: frm.doc.company, limit: 30 },
			callback(r) {
				const h = r.message || [];
				if (!h.length) {
					$out.html(`<span class="text-muted">${__("No history.")}</span>`);
					return;
				}
				$out.html(`<table class="table table-bordered table-sm">
					<thead><tr>
